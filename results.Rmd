...
if(run_everything){
  ...
  #names(df) <- gsub("_$", "", names(df))
  preselected <- preselect(check_conv, replications = 100)
  
  saveRDS(preselected, "preselected_rep.RData")
  p <- plot(preselected)
  ggsave("preselect_rep.png", p)
} else {
...
if(run_everything){
  ...
  X <- df[, c("vi", "id_sample", select_vars(preselected, cutoff = 1))]
  set.seed(747)
  # Train the model
  mf_cv <- train(y = df$yi,
                 x = X,
  ...
  VarImpPlot(final)
  png("PDP1.png")
  PartialDependence(final, vars = names(final$forest$variable.importance)[order(final$forest$variable.importance, decreasing = TRUE)][1:6], rawdata = T, pi = .95)
  dev.off()
  
  png("PDP2.png")
  PartialDependence(final, vars = names(final$forest$variable.importance)[order(final$forest$variable.importance, decreasing = TRUE)][7:length(final$forest$variable.importance)], rawdata = T, pi = .95)
  dev.off()
...
if(run_everything){
  ...
  save(rma_all, rma_es_only, rma_cv, rma_cv_es_only, rma_es_type, file = "rma_mods.RData")
} else {
  load("rma_mods.RData")
}
...
`r Words(sum(table(df[["id_sample"]]) >1))` studies reported two effect sizes.

<!--
Sample sizes ranged from `r #min(df[["n_total"]])` to `r #max(df[["n_total"]])`  
$M_n `r #report(mean(df[["n_total"]]))`$, $SD `r #report(sd(df[["n_total"]]))`$
-->
...
funnel plot asymmetry was non-significant, however;
...
"random forests". Random forests are robust to overfitting. This analysis 1)
ranks moderators in terms of their importance in predicting the effect size, 2)
visualizes the shape of the marginal association of
each moderator with the effect size (averaging over all values of the other 
moderators), and 3) provides an estimate of how much variance these moderators 
would explain in a new sample. This latter measure, $R^2_{oob}$, is interpreted
similarly to the familiar $R^2$. However, whereas $R^2$ refers to the percentage
of variance explained in the data used to estimate the model ("retrodictive"
performance), $R^2_{oob}$ estimates the percentage of variance that 
would be explained in *new* data ("predictive" performance).

To drop moderators wholly unrelated to the outcome, we first conducted a
random-effects MetaForest analysis with many (10.000) iterations,
and replicated it 100 times. Figure \@ref(fig:replicatedmf) displays the 
replicated variable importance metrics. As negative variable importance 
indicates that a variable reduced the predictive performance of the model, we 
dropped all moderators that consistently displayed negative variable importance 
(all variables below "measurement.final").

```{r replicatedmf, echo=FALSE, fig.cap="Replicated MetaForest for variable preselection"}
plot(preselected, label_elements = newnames)
```

Next, we tuned the MetaForest analysis with the remaining moderators. Tuning 
means identifying the optimal settings for the analysis, based on the smallest 
10-fold cross-validated prediction error [RSME, @hastieElementsStatisticalLearning2009].
The optimal model used `r c("random-effects", "fixed-effects", "uniform")[as.numeric(mf_cv$results$whichweights[which.min(mf_cv$results$RMSE)])]` weights, `r mf_cv$results$mtry[which.min(mf_cv$results$RMSE)]` candidate variables at each split,
...
Figure \@ref(fig:varimp) shows the relative variable importance of the moderators
in the final model. Figures \@ref(fig:pdp1) and
\@ref(fig:pdp2) show the marginal relationship of each moderator
with the effect size, averaging over all the other moderators.

```{r varimp, echo=FALSE, fig.cap="Variable importance for final model", warning=FALSE}
VarImpPlot(final, label_elements = newnames)
```

```{r pdp1, echo=FALSE, fig.cap="Partial dependence plot 1", warning=FALSE}
PartialDependence(final, vars = names(final$forest$variable.importance)[order(
  final$forest$variable.importance, decreasing = TRUE)][1:6],
  rawdata = T, pi = .95, label_elements = newnames)

```

```{r pdp2, echo=FALSE, fig.cap="Partial dependence plot 2", warning=FALSE}
PartialDependence(final, vars = names(final$forest$variable.importance)[order(
  final$forest$variable.importance, decreasing = TRUE)][7:length(
    final$forest$variable.importance)], rawdata = T, pi = .95, 
  label_elements = newnames)
```
...
Because some studies reported two effect sizes, we used three-level
meta-analysis to estimate the amount of within-study- and
between-studies variance [@vandennoortgateMetaanalysisMultipleOutcomes2015]. 
...
AIC and BIC for a model with within-studies variance constrained to zero were
...
We examined whether the six most influential moderators identified in the 
MetaForest analysis, were significant predictors in meta-regression (see Table
\@ref(tab:rma-modtable). Except for the type of effect size, however, none of 
the moderators had significant effects in a linear model. We performed
cross-validation to obtain estimates of the predictive performance in new data
for the meta-regression 
model with the six most influential moderators, and a model with only the type
of effect size as predictor. The cross-validated $R^2_{cv}$ was lower for the 
model with the six most influential moderators, $R^2_{cv} `r report(rma_cv$results$Rsquared)`$,
than for the model with only type of effect size as a predictor , $R^2_{cv} `r report(rma_cv_es_only$results$Rsquared)`$.
We thus selected the model with only type of effect size as our final 
meta-regression model. This moderator explained a significant amount of
variability in the effect size, $Q_{mods}(`r nrow(rma_es_type[["b"]])`) `r report(rma_es_type[["QM"]])`, p `r report(rma_es_type[["QMp"]])`$.

```{r rma-modtable, results = "asis"}
tmp <- motley:::printResults.rma(rma_cv$finalModel, keepCols = c("label", "est_sig", "se", "zval", "pval", "confint"))
names(tmp) <- c("Estimate", "SE", "Z", "p", "CI")
for(this_name in names(newnames)){
  tmp$Estimate <- gsub(this_name, newnames[this_name], tmp$Estimate)
}
tmp$Estimate[1] <- "Intercept"
...
As indicated in table \@ref(tab:es-by-type), the summary effect was
significantly different from zero for studies reporting a standardized mean
change rate (SMCR), but not for studies reporting a standardized mean difference
(SMD).

```{r es-by-type, results = "asis"}
tmp <- motley:::printResults.rma(rma_es_type, keepCols = c("label", "est_sig", "se", "zval", "pval", "confint"))
names(tmp) <- c("Estimate", "SE", "Z", "p", "CI")
for(this_name in names(newnames)){
  tmp$Estimate <- gsub(this_name, newnames[this_name], tmp$Estimate)
}
apa_table(tmp, caption = "Final meta-regression model, moderated by type of effect size",
          note = "* < .05, ** < .01, *** < .001")
```

### Residual heterogeneity

Significant residual heterogeneity remained after accounting for the type of
effect size, $\tau^2 `r report(rma_es_type[["tau2"]])`$, 
$SE `r report(rma_es_type[["se.tau2"]])`$,
$Q_{resid}(`r nrow(df)-nrow(rma_es_type[["b"]])`) `r report(rma_es_type[["QE"]])`$,
$p `r report(rma_es_type[["QEp"]])`$.

<!--
**Overall effect size:**

$\hat{\gamma} `r report(model.full[["b"]][1])`$, 95\% CI [`r report(model.full[["ci.lb"]], equals = FALSE)`, `r report(model.full[["ci.ub"]], equals = FALSE)`]
